...
- `scripts/fair.py` — scores the open aging datasets in `data/datasets.yml` against FAIR (Findable/Accessible/Interoperable/Reusable) → generated `docs/FAIR.md` (attacks `docs/gaps-analysis.md` G2, reproducibility). Deterministic; `--check` (drift gate), `--selftest`, and `--gate` (fails on any *unassessed* dimension — no unmeasured passes). All three are wired into `make check`.
...
- `nulls.yml` — the honest-nulls registry (longevity claims that failed/were refuted/returned a clean null; bridges gaps-analysis G4). Schema-gated by `validate.py` (`name`/`url`/`verdict`) and **rendered to `docs/NULLS.md` + a README section by `build.py`**.
- `datasets.yml` — the FAIR datasheet (open aging datasets + per-dimension FAIR assessment; bridges gaps-analysis G2). Schema-gated (`name`/`url`/`access`), scored by `fair.py` → `docs/FAIR.md`, with a README pointer via `build.py`.
- **YAML boolean/null trap (bitten twice):** unquoted `null`, `yes`, `no`, `on`, `off` are parsed as `None`/`True`/`False`, not strings — so **always quote** enum-ish values (`verdict: "null"`, `findable: "yes"`). `fair.py` also normalizes booleans defensively, but quote at the source.
